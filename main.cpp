...
int main()
{
	...
	while (!WindowShouldClose() and !fin)
	{
		...
		if (IsMouseButtonDown(MOUSE_BUTTON_LEFT) and matriz_colision[raton_celda.x][raton_celda.y] == 0
			and num_bloques > 0)
		{
			matriz_colision[raton_celda.x][raton_celda.y] = 3;
			num_bloques--;
		}
		if (debug_mode and IsMouseButtonDown(MOUSE_BUTTON_RIGHT))
		{
			if(matriz_colision[raton_celda.x][raton_celda.y])
				num_bloques++;;
		...
		if(num_bloques > 0)
			DrawRectangleLines(raton_celda.x * tam_celda, raton_celda.y * tam_celda, tam_celda, tam_celda, GREEN);

		//fin 2D
		EndMode2D();

		// Interfaz
		sprintf_s(str_buffer, "Bloques %d", num_bloques);
		DrawTextureEx(bloque_text, { 20, 50 }, 0.0f, 2.0f, WHITE);
		DrawText(str_buffer, 60, 50, 32, bloques_color);

		sprintf_s(str_buffer, "Bateria");
		DrawTextureEx(bateria_text, { 20, 100 }, 0.0f, 2.0f, WHITE);
		DrawText(str_buffer, 60, 100, 32, bateria_color);
		...
		if (tutorial)
		{
			if (tutorial_cont > -900)
			{
				...
				{
					sprintf_s(str_buffer, "<A> Izquierda, <D> Derecha, <SPACE> Saltar");
					DrawText(str_buffer, 620, 70, 32, YELLOW);
				}

				if (tutorial_cont > -400 and tutorial_cont < -180)
				{
					sprintf_s(str_buffer, "Emm... y podrias apagar la linterna con <F>");
					DrawText(str_buffer, 620, 70, 32, YELLOW);
				}
				if (tutorial_cont > -300 and tutorial_cont < -180)
				{
					sprintf_s(str_buffer, "La bateria se gasta...");
					DrawText(str_buffer, 620, 110, 32, YELLOW);
				}

				if (tutorial_cont > -150)
				{
					sprintf_s(str_buffer, "Encuentra la salida. Suerte!!");
					DrawText(str_buffer, 620, 70, 32, YELLOW);
				}
			}
		...
		if (musicote and tutorial_cont < 200)
		{
			DrawRectangle(600, 50, 1200, 150, BLACK);
			DrawRectangleLinesEx({ 600, 50, 1200, 150 }, 4.0f, YELLOW);
			sprintf_s(str_buffer, "<CLICK IZQUIERDO> Poner Bloques");
			DrawText(str_buffer, 620, 70, 32, YELLOW);
			sprintf_s(str_buffer, "Cuidado, son limitados y no se quitan.");
			DrawText(str_buffer, 620, 110, 32, YELLOW);
			tutorial_cont++;
		}
		...
		if (victoria)
		{
			DrawRectangle(600, 50, 1200, 150, BLACK);
			DrawRectangleLinesEx({ 600, 50, 1200, 150 }, 4.0f, YELLOW);
			sprintf_s(str_buffer, "Enhorabuena!!");
			DrawText(str_buffer, 1000, 75, 48, YELLOW);
		...
		if (debug_mode)
		{
			sprintf_s(str_buffer, "Filas: %d Columnas: %d", filas, columnas);
			DrawText(str_buffer, 1600, 20, 26, WHITE);
			sprintf_s(str_buffer, "Raton (%d, %d)", (int)raton.x, (int)raton.y);
			DrawText(str_buffer, 1600, 75, 26, WHITE);
			sprintf_s(str_buffer, "Celda (%d, %d)", raton_celda.x, raton_celda.y);
			DrawText(str_buffer, 1600, 100, 26, RED);
			sprintf_s(str_buffer, "Jugador (%d,%d)", (int)jug.cuerpo.x, (int)jug.cuerpo.y);
			DrawText(str_buffer, 1600, 150, 26, WHITE);
			sprintf_s(str_buffer, "Celda (%d,%d)", jug.celda.x, jug.celda.y);
			DrawText(str_buffer, 1600, 175, 26, RED);
			jug.dibujar_estado();
		}
